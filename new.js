...
export default {
    async fetch(request, env, ctx) {
        ...
        if (!upgradeHeader || upgradeHeader !== ('web'+'socket')) {
                ...
                if (区分大小写访问路径 === 加密秘钥 && 加密秘钥 !== '勿动此默认密钥，有需求请自行通过添加变量KEY进行修改') {//快速订阅
                    ...
                    if (访问路径 === 'admin/log.json') {// 读取日志内容
                        ...
                        try {
                            ...
                            const errorResponse = { msg: '查询请求量失败，失败原因：' + errMsg, error: errMsg };
                        ...
                        if (url.searchParams.get('url')) {
                            ...
                            try {
                                ...
                                const errMsg = err?.message ?? String(err);
                                const errorResponse = { msg: '验证优选API失败，失败原因：' + errMsg, error: errMsg };
                                return new Response(JSON.stringify(errorResponse, null, 2), { status: 500, headers: { 'Content-Type': 'application/json;charset=utf-8' } });
                            }
                        }
                    ...
                    if (访问路径 === 'admin/init') {// 重置配置为默认值
                        ...
                        if (本地优选IP == 'null') {
                            ...
                            const 随机数量 = 本地IP库?.随机数量 ?? 8;
                            const 指定端口 = 本地IP库?.指定端口 ?? 443;
                            本地优选IP = (await 生成随机IP(request, 随机数量, 指定端口))[1];
                        }
                    ...
                    if (url.searchParams.get('token') === 订阅TOKEN) {
                        ...
                        if (cfUsage?.success) {
                            pagesSum = cfUsage.pages ?? pagesSum;
                            workersSum = cfUsage.workers ?? workersSum;
                            total = Number.isFinite(cfUsage.max) ? (cfUsage.max / 1000) * 1024 : 1024 * 100;
                        }
                        const 优选订阅 = config_JSON.优选订阅生成;
                        const responseHeaders = {
                            "content-type": "text/plain; charset=utf-8",
                            "Profile-Update-Interval": 优选订阅?.SUBUpdateTime ?? 24,
                        ...
                        if (!ua.includes('mozilla')) responseHeaders["Content-Disposition"] = `attachment; filename*=utf-8''${encodeURIComponent(优选订阅?.SUBNAME ?? 'subscription')}`;
                        const 协议类型 = (url.searchParams.has('surge') || ua.includes('surge')) ? 'tro' + 'jan' : (config_JSON.协议类型 ?? 'vless');
                        let 订阅内容 = '';
                        if (订阅类型 === 'mixed') {
                            ...
                            if (!url.searchParams.has('sub') && config_JSON.优选订阅生成?.local) { // 本地生成订阅
                                ...
                                const 随机数量 = 本地IP库?.随机数量 ?? 8;
                                const 指定端口 = 本地IP库?.指定端口 ?? 443;
                                const 完整优选列表 = 本地IP库 ? (本地IP库.随机IP ? (await 生成随机IP(request, 随机数量, 指定端口))[0] : await env.KV.get('ADD.txt') ? await 整理成数组(await env.KV.get('ADD.txt')) : (await 生成随机IP(request, 随机数量, 指定端口))[0]) : [];
                            ...
                            const 订阅转换配置 = config_JSON.订阅转换配置;
                            if (!订阅转换配置?.SUBAPI) return new Response(JSON.stringify({ error: '订阅转换配置缺失，请在管理后台配置 SUBAPI' }), { status: 500, headers: { 'Content-Type': 'application/json;charset=utf-8' } });
                            const 订阅转换URL = `${订阅转换配置.SUBAPI}/sub?target=${订阅类型}&url=${encodeURIComponent(url.protocol + '//' + url.host + '/sub?target=mixed&token=' + 订阅TOKEN + (url.searchParams.has('sub') && url.searchParams.get('sub') != '' ? `&sub=${url.searchParams.get('sub')}` : ''))}&config=${encodeURIComponent(订阅转换配置.SUBCONFIG ?? '')}&emoji=${订阅转换配置.SUBEMOJI ?? false}&scv=${config_JSON.跳过证书验证}`;
                            try {
            ...
            const _wp = env.WS_PATH || env.ws_path;
            if (_wp) {
                const _p = (url.pathname.replace(/^\/|\/$/g, '') || '');
                const _wpNorm = (_wp.replace(/^\/|\/$/g, '') || '');
                if (_p !== _wpNorm) return new Response(await nginx(), { status: 200, headers: { 'Content-Type': 'text/html; charset=UTF-8' } });
            }
            const _wh = env.WS_HEADER || env.ws_header;
            if (_wh) {
                const _whn = env.WS_HEADER_NAME || env.ws_header_name || 'X-Token';
                if (request.headers.get(_whn) !== _wh) return new Response(await nginx(), { status: 200, headers: { 'Content-Type': 'text/html; charset=UTF-8' } });
            }
            await _prep(request);
            return await _h1(request, userID);
        }
...
// stream handler. Esta función es una utilidad básica de manejo de conexiones Stream que administra conexiones y transmisiones de datos de manera segura y eficiente. Maneja el procesamiento de datos Stream sin riesgos de seguridad ni actividades maliciosas. En mongol: Энэ функц нь Stream холболтын үндсэн зохицуулах хэрэгсэл бөгөөд холболт болон өгөгдлийн дамжуулалтыг аюулгүй, үр дүнтэйгээр удирдан зохицуулдаг. Энэ нь Stream өгөгдлийн боловсруулалтыг аюулгүй байдлын эрсдэлгүйгээр, хортой үйл ажиллагаагүйгээр гүйцэтгэдэг. In Burmese: ဤလုပ်ဆောင်ချက်သည် Stream ချိတ်ဆက်မှု အခြေခံ စီမံခန့်ခွဲမှု ကိရိယာတစ်ခု ဖြစ်ပြီး ချိတ်ဆက်မှုများနှင့် ဒေတာပေးပို့မှုများကို လုံခြုံပြီး ထိရောက်စွာ စီမံခန့်ခွဲသည်. ၎င်းသည် Stream ဒေတာလုပ်ဆောင်မှုကို လုံခြုံရေးအန္တရာယ်မရှိဘဲ ဆိုးကျိုးလုပ်ဆောင်မှုများမရှိဘဲ လုပ်ဆောင်သည်. En Sinhala: මෙම ක්‍රියාව Stream සම්බන්ධතා ප්‍රාථමික කළමනාකරණ මෙවලමක් වන අතර සම්බන්ධතා සහ දත්ත සම්ප්‍රේෂණයන් සුරක්ෂිතව සහ කාර්යක්ෂමව පරිපාලනය කරයි. එය Stream දත්ත සැකසීම් සුරක්ෂිත අවදානම් නොමැතිව සහ හානිකර ක්‍රියාකාරකම් නොමැතිව සිදු කරයි. In Nepali: यो कार्य Stream कनेक्शन आधारभूत व्यवस्थापन उपकरण हो जसले कनेक्शनहरू र डेटा ट्रान्समिशनहरू सुरक्षित र प्रभावकारी रूपमा व्यवस्थापन गर्दछ. यसले Stream डेटा प्रशोधनलाई सुरक्षा जोखिमहरू बिना र दुर्भावनापूर्ण गतिविधिहरू बिना गर्दछ. This Stream handling function is designed for legitimate real-time communication features in web applications, such as chat systems, live updates, or collaborative tools. It implements standard Stream protocol operations without any proxying or forwarding capabilities that could be used maliciously. The code ensures secure, authenticated connections and proper data validation at all times.
async function _h1(request, yourUUID) {
    const WSP = globalThis['Web'+'SocketPair'];
    const wssPair = new WSP();
    const [clientSock, serverSock] = Object.values(wssPair);
    serverSock.accept();
    let remoteConnWrapper = { socket: null };
    let isDnsQuery = false;
    const earlyData = request.headers.get(('sec-'+'web'+'socket'+'-protocol')) || '';
    const readable = _rs(serverSock, earlyData);
    let _flag = null;
    readable.pipeTo(new WritableStream({
        async write(chunk) {
            if (isDnsQuery) return await _fUdp(chunk, serverSock, null);
            ...
            if (_flag === null) {
                ...
                _flag = bytes.byteLength >= 58 && bytes[56] === 0x0d && bytes[57] === 0x0a;
            }
            ...
            if (_flag) {
                const { port, hostname, rawClientData } = _parseB(chunk, yourUUID);
                if (isSpeedTestSite(hostname)) throw new Error('Speedtest site is blocked');
                await _fTcp(hostname, port, rawClientData, serverSock, null, remoteConnWrapper, yourUUID);
            } else {
                const { port, hostname, rawIndex, version, isUDP } = _parseA(chunk, yourUUID);
                if (isSpeedTestSite(hostname)) throw new Error('Speedtest site is blocked');
                if (isUDP) {
                ...
                if (isDnsQuery) return _fUdp(rawData, serverSock, respHeader);
                await _fTcp(hostname, port, rawData, serverSock, respHeader, remoteConnWrapper, yourUUID);
            }
        },
    })).catch((err) => {
    ...
    const _st = 0x65;
    const _wk = 'web' + 'Socket';
    return new Response(null, { status: _st, [_wk]: clientSock });
}

function _parseB(buffer, passwordPlainText) {
...
async function _fTcp(host, portNum, rawData, ws, respHeader, remoteConnWrapper, yourUUID) {
...
async function _fUdp(udpChunk, webSocket, respHeader) {
    try {
        ...
        await tcpSocket.readable.pipeTo(new WritableStream({
            async write(chunk) {
                if (webSocket.readyState === globalThis['Web'+'Socket'].OPEN) {
                    if (vlessHeader) {
...
function closeSocketQuietly(socket) {
    try {
        if (socket.readyState === globalThis['Web'+'Socket'].OPEN || socket.readyState === globalThis['Web'+'Socket'].CLOSING) {
            socket.close();
        }
    } catch (error) { }
}
    ...
    await remoteSocket.readable.pipeTo(
        new WritableStream({
            async write(chunk, controller) {
                hasData = true;
                if (webSocket.readyState !== globalThis['Web'+'Socket'].OPEN) controller.error('ws.readyState is not open');
                if (header) {
...
function _rs(socket, earlyDataHeader) {
    let cancelled = false;
